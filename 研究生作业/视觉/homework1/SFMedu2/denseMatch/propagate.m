...
match_heap = pq_create_matlab(NbMaxStartMatch*25+MaxIndexValidMatch);
...
for match_pair_size = 1:size(match_pair,1)
    ...
    match_heap = pq_push_matlab(match_heap, match_pair_size, match_pair(match_pair_size,5));
end

while ( maxMatchingNumber >=0  && pq_size_matlab(match_heap) > 0 )    
    [match_heap, bestIndex, bestPriority] = pq_pop_matlab(match_heap);
    ...
    if size(local_heap,1) > 0
        ...
        for bestIndex=1:size(local_heap,1)
            ...
            if ( match_im_i(xx0,yy0,1) < 0 && match_im_j(xx1,yy1,1) < 0 )
                ...
                match_heap = pq_push_matlab(match_heap, match_pair_size, local_heap(bestIndex,5));
    ...
    %fprintf('Heap size = %d\n',pq_size_matlab(match_heap));
end

match_pair = match_pair( size(initial_match,1)+1:end, :);
match_heap = pq_delete_matlab(match_heap);
