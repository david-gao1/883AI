...
try
    vl_setup('quiet');
    % Clear function cache and verify MEX files are loaded
    clear('vl_sift');
    pause(0.1);  % Give MATLAB time to process
    if exist('vl_sift', 'file') == 3
        fprintf('VLFeat MEX files loaded successfully.\n');
    else
        ...
        if strcmp(currentArch, 'mexmaca64')
            error(['\n' ...
                ...
                '========================================\n' ...
                'MATLAB is running as ARM64 (Apple Silicon native),\n' ...
                'but VLFeat MEX files are x86_64.\n' ...
                'ARM64 MATLAB cannot load x86_64 MEX files.\n\n' ...
                'SOLUTION: Run MATLAB under Rosetta 2 (x86_64 mode)\n' ...
                '1. Quit MATLAB\n' ...
                '2. In Terminal, run:\n' ...
                '   arch -x86_64 /Applications/MATLAB_R202*.app/bin/matlab\n' ...
                '   (adjust path to your MATLAB installation)\n' ...
                '3. Or: Right-click MATLAB in Finder > Get Info >\n' ...
                '   Check "Open using Rosetta"\n' ...
                '========================================\n']);
        else
            error('VLFeat MEX files not found. Check vl_setup.');
        end
    end
catch ME
    % Check if it's an architecture mismatch error
    if contains(ME.message, 'ARCHITECTURE MISMATCH')
        error(ME.message);
    end
    warning('Error initializing VLFeat: %s', ME.message);
    % Try to manually add MEX path (won't work for ARM64, but try anyway)
    mexPath = fullfile('matchSIFT', 'vlfeat', 'toolbox', 'mex', 'mexmaci64');
    if exist(mexPath, 'dir')
        addpath(mexPath, '-begin');
        clear('vl_sift');  % Clear cache
        pause(0.1);  % Wait for MATLAB to process
        fprintf('Manually added MEX path: %s\n', mexPath);
        if exist('vl_sift', 'file') ~= 3
            currentArch = mexext;
            if strcmp(currentArch, 'mexmaca64')
                error(['\n' ...
                    'Cannot load x86_64 MEX files in ARM64 MATLAB.\n' ...
                    'Please restart MATLAB under Rosetta 2 (x86_64 mode).\n' ...
                    'See instructions above.\n']);
            else
                warning('vl_sift still not found after manual path addition.');
            end
        end
    else
        error('Cannot find VLFeat MEX files. Please check installation.');
    end
end
...
% Using images from myself folder
% frames.images{1}='myself/1.png';
% frames.images{2}='myself/2.png';
% frames.images{3}='myself/3.png';
% frames.images{4}='myself/4.png';
% frames.images{5}='myself/5.png';

% Original test images (commented out)
