...
fprintf('initial error = %f\n', 2*sqrt(sum(double(residuals).^2)/length(residuals)));
...
[vec,resnorm,residuals,exitflag] = lsqnonlin(@(x) reprojectionResidual(graph.ObsIdx,graph.ObsVal,px,py,f,x), double([Mot(:); Str(:)]),[],[],options);
...
if exist('adjustFocalLength','var') && adjustFocalLength
    ...
    [vec,resnorm,residuals,exitflag] = lsqnonlin(@(x) reprojectionResidual(graph.ObsIdx,graph.ObsVal,px,py,x), double([f; Mot(:); Str(:)]),[],[],options);
